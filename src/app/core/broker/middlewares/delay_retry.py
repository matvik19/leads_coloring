...
class DelayRetryMiddleware(TaskiqMiddleware):
    """Middleware to add retries with delays."""
    ...
    async def on_error(
        ...
        timeout_delay = int(message.labels.get("timeout_delay", self.default_retry_delay))

        # Getting number of previous retries.
        retries = int(message.labels.get("_retries", 0)) + 1
        # Добавление лейблов к таску
        kicker.with_labels(_retries=retries, delay=timeout_delay * retries)
